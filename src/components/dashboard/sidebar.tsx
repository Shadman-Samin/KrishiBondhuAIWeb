...
export function Sidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  return (
    <aside
      className={cn(
        "hidden md:flex flex-col h-screen border-r border-black/10 dark:border-white/10 bg-white/80 dark:bg-[#0B150F]/80 backdrop-blur-xl transition-all duration-300 sticky top-0",
        collapsed ? "w-[68px]" : "w-64",
      )}
    >
      <div
        className={cn(
          "flex items-center gap-2 border-b border-black/10 dark:border-white/10 px-4 h-16",
      ...
      <div className="border-t border-black/10 dark:border-white/10 px-2 py-2 space-y-1">
...
export function MobileSidebar({
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex flex-col w-64 bg-white/90 dark:bg-[#0B150F]/90 backdrop-blur-xl border-r border-black/10 dark:border-white/10 transform transition-transform duration-300 md:hidden",
      ...
      >
        <Link
          to="/"
          onClick={() => onOpenChange(false)}
          className="flex items-center gap-2 border-b border-black/10 dark:border-white/10 px-4 h-16"
        >
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-primary shadow-glow">
            <Leaf className="h-5 w-5 text-primary-foreground" />
          </div>
